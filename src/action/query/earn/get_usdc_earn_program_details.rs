    ...
    let resp = GetUsdcEarnProgramResp {
        data: match address {
            Some(addr) => {
                ...
                available.usd_amount = available.usd_amount.checked_div(usdc_usd_price).unwrap();

                let mut staked = querier.get_staked_balance(addr.clone(), asset.clone())?;
                staked.usd_amount = staked.usd_amount.checked_div(usdc_usd_price).unwrap();
                
                let mut borrowed = querier.get_borrowed_balance(addr.clone())?;
                borrowed.usd_amount = borrowed.usd_amount.checked_div(usdc_usd_price).unwrap();
